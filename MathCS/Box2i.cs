...
namespace TCMath
{
	...
	public struct Box2i
	{
		...
		////////////////////////////////////////////////////////////////////////
		public bool DoesIntersectBox( Box2i testBox )
		{
		...
		////////////////////////////////////////////////////////////////////////
		public bool IntelliSnapToBox( Box2i snapBox )
		{
			return IntelliSnapToBox( snapBox, 10 );
		}
		...
		////////////////////////////////////////////////////////////////////////
		public bool GetIntelliSnapOffset( Box2i snapBox, int threshold, ref Vector2i snapVect )
		{
			// First test if the corners would snap to each other
			float threshSq = (float)(threshold * threshold);
			...
			for( int snapIdx = 0; snapIdx < 4; ++snapIdx )
			{
				for( int boxIdx = 0; boxIdx < 4; ++boxIdx )
				{
					...
					Vector2 diffV = new Vector2( this[boxIdx].x - snapBox[snapIdx].x,
													this[boxIdx].y - snapBox[snapIdx].y );
					float distSq = diffV.GetMagSq();
					if( distSq > threshSq )
						continue;
			...
			if( ourVertInterval.IsIntervalIntersecting( snapVertInterval ) )
			{
				...
				int dist = snapBox.pos.x - pos.x;
				if( Math.Abs( dist ) < Math.Abs( minMoveDist ) )
					minMoveDist = dist;

				// Test our left to snap right
				dist = (snapBox.pos.x + snapBox.dims.x) - pos.x;
				if( Math.Abs( dist ) < Math.Abs( minMoveDist ) )
					minMoveDist = dist;
			...
			if( ourHorzInterval.IsIntervalIntersecting( snapHorzInterval ) )
			{
				...
				dist = snapBox.pos.x - (pos.y + dims.y);
				if( Math.Abs( dist ) < Math.Abs( minDist ) )
					minDist = dist;
		...
		public Point2i this [ int index ]
		{
			...
			{
				if( index == 0 )
					return new Point2i( pos.x + dims.x, pos.y );
				else if( index == 1 )
					return pos;
				else if( index == 2 )
					return new Point2i( pos.x, pos.y + dims.y );

				return new Point2i( pos.x + dims.x, pos.y + dims.y );
			}
		}
		...
		{
			return String.Format( "({0},{1},{2},{3})", pos.x, pos.y, dims.x, dims.y );
		}
		...
		///////////////////////////////////////////////////////////////////////
		public static Box2i Parse( string str )
		{
			const string ERROR_STR = "Box2i string not formatted correctly";
			...
			for( int paramIndex = 0; paramIndex < delims.Length; ++paramIndex )
			{
				...
				int paramValue = int.Parse( valStr );
		...
		{
			pos.x = binReader.ReadInt32();
			pos.y = binReader.ReadInt32();
			dims.x = binReader.ReadInt32();
			dims.y = binReader.ReadInt32();
		}
	}
}
