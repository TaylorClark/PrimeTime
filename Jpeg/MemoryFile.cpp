...
namespace corona {
	...
	COR_EXPORT(File*) CorCreateMemoryFile(const void* buffer, int size, bool copyMem )
	{
		if (size && !buffer) {
			return 0;
		}
		...
		return new MemoryFile(buffer, size, copyMem);
	}
	...
	MemoryFile::MemoryFile(const void* buffer, int size, bool copyMem) {
