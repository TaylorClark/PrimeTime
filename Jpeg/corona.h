...
namespace corona {
  ...
  namespace hidden {
    ...
    COR_FUNCTION(File*) CorCreateMemoryFile(const void* buffer, int size, bool copyMem = true);
   ...
   */
  inline File* CreateMemoryFile(const void* buffer, int size, bool copyMem = true) {
    return hidden::CorCreateMemoryFile(buffer, size, copyMem );
  }
  ...
  typedef void (*WriteJpegData)( uint8* pSrcPixel, uint32 numRows, void* pUserData );
  ...
  public:
	  ...
	  virtual uint8* ReadLine() = 0;
  };

  Image* OpenJPEG(File* file, WriteJpegData callbackFunc = 0, void* pUserData = 0);
