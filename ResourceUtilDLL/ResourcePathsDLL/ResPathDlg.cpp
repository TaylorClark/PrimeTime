...
void CResPathDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_RESPATHEDIT, m_ctlPathRes);
	DDX_Control(pDX, IDC_GAMEPATHEDIT2, m_ctlPathGame);
}


BEGIN_MESSAGE_MAP(CResPathDlg, CDialog)
	ON_BN_CLICKED(IDC_BUTTONOK, &CResPathDlg::OnBnClickedButtonok)
	ON_BN_CLICKED(IDC_BUTTONCANCEL, &CResPathDlg::OnBnClickedButtoncancel)
